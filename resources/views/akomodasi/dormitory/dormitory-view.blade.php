                    ...
                    <div class="col-md-12">
                        ...
                        <div class="d-flex flex-row mt-2">
                            <div class="col-8 p-3 mb-1">
                                <p class="h2 font-weight-bold">{{ ucwords($dormitory->nama_dormitory) }}</p>
                                <p class="h5 text-gray-600 font-weight-bold mb-4">{{ ucwords($dormitory->alamat) }}</p>
                            </div>
                        </div>
                        ...
                        <div class="d-flex mb-1">
                            <div class="flex-1">
                                ...
                                <input disabled class="form-control bg-white" id="jarak" name="jarak" type="text"
                                    value="{{ $dormitory->jarak_ke_yarsi_mobil }} km" required />
                            </div>

                            <div class="flex-1">
                                <label class="form-label" for="jarak" value="Jarak">Distance Motorcycle</label>
                                <input disabled class="form-control bg-white" id="jarak" name="jarak" type="text"
                                    value="{{ $dormitory->jarak_ke_yarsi_motor }} KM" required />
                            </div>

                            <div class="flex-1">
                                <label class="form-label" for="jarak" value="Jarak">Distance Walk</label>
                                <input disabled class="form-control bg-white" id="jarak" name="jarak" type="text"
                                    value="{{ $dormitory->jarak_ke_yarsi_jk }} KM" required />
                            </div>
                        </div>
                        ...
                        <div class="d-flex mb-1 ">
                            <div class="flex-1">
                                ...
                                <input disabled id="ipl" name="ipl" type="text"
                                    value="Rp. {{ number_format($dormitory->ipl, 0, ',', '.') }}"
                            ...
                            <div class="flex-1">
                                ...
                                <input disabled id="listrik" name="listrik" type="text"
                                    value="Rp. {{ number_format($dormitory->listrik, 0, ',', '.') }}"
                                    class="form-control number-format" required />
                            </div>
                            <div class="flex-1">
                                <label for="wifi">Wifi</label>
                                <input disabled id="wifi" name="wifi" type="text"
                                    value="Rp. {{ number_format($dormitory->wifi, 0, ',', '.') }}"
                                    class="form-control number-format" required />
                            </div>
                        </div>
                        <div>
                            <p class="h6 font-weight-bold" for="kelebihan" value="Kelebihan">Adventages</p>
                            <textarea style="resize: none;" class="form-control" disabled id="kelebihan" name="kelebihan" rows="4">{{ $dormitory->kelebihan }}</textarea>
                        </div>
                        <div>
                            <p class="h6 font-weight-bold" for="kekurangan" value="Kekurangan">Disadventages</p>
                            <textarea style="resize: none;" class="form-control" disabled id="kekurangan" name="kekurangan" rows="4">{{ $dormitory->kekurangan }}</textarea>
                        </div>
                        @if (Auth::user()->role === 'user' && $pilihDormitory)
                            <div class="d-flex justify-content-center mt-1">
                                <button type="button" class="btn btn-primary w-25 text-center pilih-dormitory"
                                    style="cursor: pointer" value="{{ $dormitory->id_dormitory }}">Pilih</button>
                            </div>
                        @endif
